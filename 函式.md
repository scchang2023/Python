...
```
``` python
# 在想要裝飾的函式前面加上 @decorator_name 來取代手動
@document_it
def add_ints(a, b):
    return a+b
add_ints(3, 5)
...
```
如果直接修改它，它會修改另一個也叫做animal 的變數，但是該變數是在函式裡面的
``` python
animal = 'fruitbat'
def change_local():
    animal = 'wombat'
...
```
``` python
inside change_local: wombat 1622495032560
fruitbat 1622495053488
```
使用 global 關鍵字存取全域變數
``` python
animal = 'fruitbat'
def change_print_global():
    global animal
    animal = 'wombat'
    print('inside change_print_global:', animal)
print(animal)
change_print_global()
print(animal)
```
``` python
fruitbat
inside change_print_global: wombat
wombat
```
## 在名稱內使用 _ 與 __
python 保留在名稱開頭與結尾使用兩個底線 __ 的寫法，不能在自己的變數使用它們。
## 遞迴 recursion
函式自己呼叫自己
## 非同步函式
python 3.5 加入關鍵字 async 與 await 來定義和執行非同步函式
## 例外
...
```
``` python
need a position between 0 and 3 but got 5
```
